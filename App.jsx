        ...
        <Modal title={editing ? "Editar Pedido" : "Novo Pedido"} onClose={() => setShowModal(false)}>
          ...
          <div style={{ marginBottom: 14, position: "relative" }}>
            ...
            <input
              ...
              onFocus={e => e.target.style.borderColor = C.primary}
              onBlur={e => setTimeout(() => e.target.style.borderColor = C.lavenderMid, 150)}
            />
            {form.clientName && clients.filter(c => c.name.toLowerCase().includes(form.clientName.toLowerCase())).length > 0 && (
              <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: C.white, borderRadius: 10, boxShadow: "0 4px 20px rgba(123,79,166,0.2)", border: `1.5px solid ${C.lavenderMid}`, zIndex: 100, maxHeight: 180, overflowY: "auto" }}>
                {clients.filter(c => c.name.toLowerCase().includes(form.clientName.toLowerCase())).map(c => (
                  <div key={c.id}
                    onMouseDown={() => setForm(f => ({ ...f, clientName: c.name }))}
