...
// SEÇÃO: DASHBOARD
// ════════════════════════════════════════════════════════
const Dashboard = ({ orders, clients, catalog, setPage }) => {
  ...
  const estimatedProfit = orders
    ...
    .reduce((s, o) => s + (parseFloat(o.finalPrice) || 0), 0) * 0.4;
  ...
  return (
    <div style={{ minHeight: "100vh", margin: "-32px -32px -80px", padding: "32px 28px 100px" }}>
      {/* HEADER */}
      ...
      <div style={{
        ...
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 11, color: "rgba(201,168,255,0.7)", fontWeight: 600, letterSpacing: 1.2, textTransform: "uppercase" }}>Lucro Estimado</div>
          <div style={{ fontSize: 26, fontWeight: 900, color: "#7FFFA8", letterSpacing: -0.5 }}>R$ {estimatedProfit.toFixed(2).replace(".", ",")}</div>
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", marginTop: 2 }}>sobre pedidos finalizados</div>
        </div>
      </div>
...
const Calculator = () => {
  ...
  const [machineCost, setMachineCost] = useState("5");
  ...
  const totalCost = filamentCost + parseFloat(energy || 0) + parseFloat(machineCost || 0) + laborCost + parseFloat(extraCosts || 0);
  const finalPrice = totalCost * (1 + parseFloat(margin || 0) / 100);
  const profit = finalPrice - totalCost;

  const row = (label, value, bold) => (
    <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: `1px solid ${C.lavender}`, fontWeight: bold ? 700 : 400 }}>
      <span style={{ color: bold ? C.primaryDark : C.textMid }}>{label}</span>
      <span style={{ color: bold ? C.primary : C.text }}>R$ {value.toFixed(2).replace(".", ",")}</span>
    </div>
  );
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <div style={{ background: C.white, borderRadius: 14, padding: 22, boxShadow: "0 2px 12px rgba(123,79,166,0.08)" }}>
          <h4 style={{ color: C.primaryDark, margin: "0 0 16px", fontSize: 14, textTransform: "uppercase", letterSpacing: 1 }}>🧵 Filamento</h4>
          <Field label="Preço do Filamento (R$/kg)" type="number" value={filamentPrice} onChange={setFilamentPrice} placeholder="90" />
          <Field label="Gramas Usadas (g)" type="number" value={gramsUsed} onChange={setGramsUsed} placeholder="50" />
          <h4 style={{ color: C.primaryDark, margin: "16px 0 16px", fontSize: 14, textTransform: "uppercase", letterSpacing: 1 }}>⚡ Energia & Máquina</h4>
          <Field label="Custo de Energia (R$)" type="number" value={energy} onChange={setEnergy} placeholder="2.00" step="0.01" />
          <Field label="Desgaste da Máquina (R$)" type="number" value={machineCost} onChange={setMachineCost} placeholder="5.00" step="0.01" />
        </div>
        <div style={{ background: C.white, borderRadius: 14, padding: 22, boxShadow: "0 2px 12px rgba(123,79,166,0.08)" }}>
          <h4 style={{ color: C.primaryDark, margin: "0 0 16px", fontSize: 14, textTransform: "uppercase", letterSpacing: 1 }}>🖌️ Mão de Obra</h4>
          <Field label="Horas Trabalhadas (h)" type="number" value={laborHours} onChange={setLaborHours} placeholder="1" step="0.5" />
          <Field label="Valor por Hora (R$/h)" type="number" value={laborRate} onChange={setLaborRate} placeholder="25" />
          <h4 style={{ color: C.primaryDark, margin: "16px 0 16px", fontSize: 14, textTransform: "uppercase", letterSpacing: 1 }}>📦 Extras & Lucro</h4>
          <Field label="Custos Extras — tinta, lixa... (R$)" type="number" value={extraCosts} onChange={setExtraCosts} placeholder="0" step="0.01" />
          <Field label="Margem de Lucro (%)" type="number" value={margin} onChange={setMargin} placeholder="50" />
        </div>
      </div>
      <div style={{ background: `linear-gradient(135deg, ${C.primaryDark}, ${C.primary})`, borderRadius: 14, padding: 24, marginTop: 16, color: "#fff" }}>
        <h4 style={{ margin: "0 0 16px", fontSize: 14, textTransform: "uppercase", letterSpacing: 1, opacity: 0.8 }}>📊 Resumo</h4>
        {row("Filamento", filamentCost)}
        {row("Energia", parseFloat(energy || 0))}
        {row("Desgaste da máquina", parseFloat(machineCost || 0))}
        {row("Mão de obra", laborCost)}
        {row("Custos extras", parseFloat(extraCosts || 0))}
        <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderTop: "1.5px solid rgba(255,255,255,0.3)", marginTop: 4 }}>
          <span style={{ opacity: 0.8 }}>Custo Total</span><span>R$ {totalCost.toFixed(2).replace(".", ",")}</span>
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderTop: "1.5px solid rgba(255,255,255,0.3)" }}>
          <span style={{ opacity: 0.8 }}>Lucro Estimado</span><span style={{ color: "#B9FFB9" }}>R$ {profit.toFixed(2).replace(".", ",")}</span>
        </div>
        <div style={{
          display: "flex", justifyContent: "space-between", alignItems: "center",
          background: "rgba(255,255,255,0.15)", borderRadius: 12, padding: "14px 18px", marginTop: 12
        }}>
          <span style={{ fontSize: 14, fontWeight: 600 }}>💰 Preço de Venda Sugerido</span>
          <span style={{ fontSize: 26, fontWeight: 800 }}>R$ {finalPrice.toFixed(2).replace(".", ",")}</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Campos */}
        <div style={{ marginBottom: 14 }}>
          <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "rgba(201,168,255,0.7)", marginBottom: 6, letterSpacing: 1, textTransform: "uppercase" }}>Usuário</label>
          <input value={user} onChange={e => setUser(e.target.value)}
            placeholder="seu usuário"
            style={{
              width: "100%", padding: "12px 16px", borderRadius: 12, boxSizing: "border-box",
              background: "rgba(255,255,255,0.07)", border: "1px solid rgba(201,168,255,0.2)",
              color: "#EEE8FF", fontSize: 14, fontFamily: "inherit", outline: "none",
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            <button onClick={() => setShowPass(v => !v)} style={{
              ...
              background: "none", border: "none", cursor: "pointer", color: "rgba(201,168,255,0.5)", fontSize: 16, padding: 0
            }}>{showPass ? "🙈" : "👁️"}</button>
          </div>
        </div>
        ...
        <button onClick={tryLogin} style={{
          ...
          background: "linear-gradient(135deg, #7B4FA6, #9B6DC5)",
          color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer",
          fontFamily: "inherit", letterSpacing: 0.3,
          boxShadow: "0 4px 20px rgba(123,79,166,0.4)",
          transition: "transform .15s, box-shadow .15s"
        }}
          onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-1px)"; e.currentTarget.style.boxShadow = "0 8px 28px rgba(123,79,166,0.55)"; }}
          onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "0 4px 20px rgba(123,79,166,0.4)"; }}
        >
          Entrar
        </button>
...
export default function App() {
  ...
  const renderPage = () => {
    switch (page) {
      case "home": return <Dashboard orders={orders} clients={clients} catalog={catalog} setPage={setPage} />;
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "linear-gradient(145deg, #1E1040 0%, #2A1658 50%, #1A2040 100%)", fontFamily: "'Segoe UI', system-ui, sans-serif", position: "relative" }}>
      ...
      <div style={{ display: "none", position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 200, background: "rgba(20,10,48,0.92)", borderTop: "1px solid rgba(201,168,255,0.12)", backdropFilter: "blur(20px)", flexDirection: "row", justifyContent: "space-around", padding: "8px 0 12px" }} id="mobile-nav">
        {nav.map(n => (
          <button key={n.id} onClick={() => setPage(n.id)} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3, background: "none", border: "none", cursor: "pointer", fontFamily: "inherit", color: page === n.id ? "#D4B8FF" : "rgba(201,168,255,0.4)", fontSize: 10, fontWeight: 600, padding: "4px 8px" }}>
            <Icon name={n.icon} size={20} color={page === n.id ? "#D4B8FF" : "rgba(201,168,255,0.4)"} />
            {n.label}
          </button>
        ))}
      </div>

      <main style={{ flex: 1, padding: "32px 32px 80px", maxWidth: 900, margin: "0 auto", width: "100%", position: "relative", zIndex: 1 }}>
