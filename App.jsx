...
const Orders = ({ orders, addOrder, updateOrder, deleteOrder, updateOrderStatus, clients }) => {
  ...
  const emptyForm = { clientName: "", productName: "", description: "", orderDate: today, deliveryDate: "", paymentMethod: "PIX", finalPrice: "", status: "Pedido Recebido", paid: false, notes: "" };
...
const Financial = ({ orders, supplies }) => {
  ...
  const generateReport = () => {
    const lines = [
      ...
      `Pedidos em Aberto:   ${unpaidOrders.length}`,
      "",
      "FINANCEIRO",
      `Total Recebido:      ${fmt(totalReceived)}`,
      `Total em Aberto:     ${fmt(totalPending)}`,
      `Total Investido:     ${fmt(totalInvested)}`,
      `Lucro Real:          ${realProfit < 0 ? "-" : ""}${fmt(realProfit)}`,
      `Situação:            ${realProfit >= 0 ? "NO LUCRO" : "PREJUÍZO"}`,
      `Margem de Lucro:     ${profitMargin}%`,
      "",
      "CLIENTES QUE PAGARAM",
      ...(payers.length === 0 ? ["Nenhum ainda."] : payers.map(([n,v]) => `${n}: ${fmt(v)}`)),
      "",
      "PAGAMENTOS EM ABERTO",
      ...(debtors.length === 0 ? ["Nenhum."] : debtors.map(o => {
        const late = o.daysLate !== null ? (o.daysLate > 0 ? ` (${o.daysLate} dias atrasado)` : " (no prazo)") : "";
        return `${o.clientName||"—"} — ${o.productName} — ${fmt(parseFloat(o.finalPrice))}${late}`;
      })),
      "",
      "INSUMOS INVESTIDOS",
      ...((supplies||[]).length === 0 ? ["Nenhum."] : (supplies||[]).map(s => `${s.name} x${s.quantity}: ${fmt(parseFloat(s.price)*parseFloat(s.quantity||1))}`)),
      `TOTAL: ${fmt(totalInvested)}`,
      "",
      "=".repeat(48),
      "LayerLab — Gestão de Impressão 3D",
    ];
    const blob = new Blob([lines.join("\n")], { type: "text/plain;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `LayerLab_${today.toLocaleDateString("pt-BR").replace(/\//g,"-")}.txt`;
    a.click();
  ...
  return (
    <div style={{ paddingBottom: 20 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        ...
        <button onClick={generateReport} style={{ padding: "10px 16px", borderRadius: 12, border: "none", background: "linear-gradient(135deg, #7B4FA6, #9B6DC5)", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
          📄 Relatório
        </button>
      </div>
...
export default function App() {
  ...
  const toDb = (o) => ({ client_name: o.clientName, product_name: o.productName, description: o.description, order_date: o.orderDate, delivery_date: o.deliveryDate, payment_method: o.paymentMethod, final_price: parseFloat(o.finalPrice) || 0, status: o.status, paid: o.paid||false, notes: o.notes });
