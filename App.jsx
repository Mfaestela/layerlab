...
function HomePage({ setPage, orders, supplies }) {
  ...
  // Mostra o valor ou esconde com bolinhas conforme o olhinho
  const money = (v) => (showValues ? fmtBRL(v) : "R$ ••••");
    ...
    <div style={{ ...s.content }}>
      ...
      <div style={{ ...s.card, marginBottom: 10, padding: "12px 16px" }}>
        ...
        <p style={{ fontSize: 11, color: C.textSoft, margin: "2px 0 0" }}>
          Vendas {money(received)} + Gastos {money(invested)}
        </p>
      </div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
        ...
        >
          ...
          <p style={{ fontSize: 10, color: C.textSoft, margin: "4px 0 0", lineHeight: 1.3 }}>
            {profit < 0 ? `Faltam ${money(Math.abs(profit))} pra empatar` : "Vendas - investimento"}
          </p>
        </div>
...
function SuppliesPage({ reloadSupplies }) {
  ...
  const [saving, setSaving] = useState(false);
  const [showArchived, setShowArchived] = useState(false); // ver arquivados
  const [suggestion, setSuggestion] = useState(null); // item arquivado sugerido
  ...
  // Itens da aba ativa, separados por arquivado/ativo
  const tabItems = items.filter(
    (i) => (i.kind || "consumivel") === activeTab && (showArchived ? i.archived : !i.archived)
  );
  // Quantos arquivados existem nesta aba (pro contador do botao)
  const archivedCount = items.filter((i) => (i.kind || "consumivel") === activeTab && i.archived).length;

  // Totais somam TUDO (ativos + arquivados) - foi gasto que tive
  ...
  // Arquivar (item acabou) ou restaurar
  async function setArchived(item, val) {
    await db.update("supplies", item.id, { archived: val });
    refreshAll();
  }
  ...
  async function save() {
    ...
    try {
      ...
      if (editing) {
        ...
        // item novo entra sempre como ativo (em estoque)
        await db.insert("supplies", { ...payload, archived: false });
      }
      setEditing(null);
      setShowNew(false);
      setSuggestion(null);
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.content}>
        ...
        {/* Explicacao da aba */}
        <p style={{ fontSize: 12, color: C.textSoft, margin: "0 0 10px 2px" }}>
        ...
        {showNew && (
          <div style={{ ...s.card, border: `1px solid ${C.purple}55`, marginBottom: 12 }}>
            ...
            <Field label="Nome *">
              <input
                ...
                onChange={(e) => {
                  ...
                  if (val.trim().length >= 2 && !editing) {
                    const match = items.find(
                      (i) => i.archived && (i.name || "").toLowerCase().trim() === val.toLowerCase().trim()
                    ) || items.find(
                      (i) => i.archived && (i.name || "").toLowerCase().includes(val.toLowerCase().trim())
                    );
            ...
            {suggestion && (
              <div
                style={{
                  background: "rgba(155,109,197,0.12)",
                  border: "1px solid rgba(155,109,197,0.3)",
                  borderRadius: 10,
                  padding: 10,
                  marginBottom: 10,
              ...
              >
                <p style={{ margin: "0 0 6px", color: C.textSoft }}>
                  Voce ja teve <b style={{ color: C.text }}>{suggestion.name}</b> (arquivado, {fmtBRL(suggestion.price)}). Reaproveitar os dados?
                </p>
                <button
                ...
                >
                  Usar nome e categoria (digito o preco novo)
                </button>
              </div>
            )}
            <Field label="Categoria">
        ...
        {Object.entries(byCategory).map(([cat, its]) => (
          <div key={cat}>
            ...
            {its.map((item) => (
              <div key={item.id} style={{ ...s.card, padding: 12, display: "flex", alignItems: "center", justifyContent: "space-between", opacity: item.archived ? 0.7 : 1 }}>
                ...
                <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
                  {item.archived ? (
                    ...
                    <button
                      ...
                      style={{ ...s.btnGhost, padding: "5px 10px", fontSize: 12 }}
                      title="Marcar que acabou (arquivar)"
                    >
                      📦
                    </button>
                  )}
                  <button onClick={async () => { if (!confirm("Deletar de vez?")) return; await db.delete("supplies", item.id); refreshAll(); }} style={{ ...s.btnDanger, padding: "5px 10px" }}>🗑️</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {tabItems.length === 0 && !showNew && (
          <div style={{ textAlign: "center", padding: 32 }}>
            <p style={{ color: C.textSoft }}>
              {showArchived
                ? "Nenhum item arquivado aqui."
                : `Nenhum ${activeTab === "equipamento" ? "equipamento" : "consumivel"} em estoque.`}
            </p>
            {!showArchived && <button style={s.btn} onClick={openNew}>+ Adicionar</button>}
          </div>
        )}
      </div>
    </div>
...
// FINANCE PAGE
// ============================================================
function FinancePage({ orders, supplies, clients = [], catalog = [], services = [] }) {
  const received = orders.filter((o) => o.paid).reduce((s, o) => s + (o.final_price || 0), 0);
  ...
  // Exporta backup completo em Excel (uma aba por tabela)
  const [exporting, setExporting] = useState(false);
  async function exportBackup() {
    if (!window.XLSX) {
      alert("Gerador de Excel ainda carregando, tente de novo em instantes.");
      return;
    }
    setExporting(true);
    try {
      ...
      // Helper: transforma lista em aba, tratando JSON de products/services
      const addSheet = (name, rows) => {
        const data = rows.length ? rows : [{ vazio: "sem registros" }];
        const ws = XLSX.utils.json_to_sheet(data);
        XLSX.utils.book_append_sheet(wb, ws, name);
      };

      // Clientes
      addSheet("Clientes", clients.map((c) => ({
        Nome: c.name || "",
        Telefone: c.phone || "",
        Email: c.email || "",
        Endereco: c.address || "",
      ...
      // Pedidos
      addSheet("Pedidos", orders.map((o) => {
        let prods = "";
        try {
          const p = JSON.parse(o.products || "[]");
          prods = p.map((x) => `${x.name} x${x.qty}`).join("; ");
        } catch { prods = ""; }
        return {
          ...
          Status: o.status || "",
          Valor_Final: o.final_price || 0,
          Lucro: o.profit || 0,
          Pago: o.paid ? "Sim" : "Nao",
          Forma_Pagamento: o.payment_method || "",
          Data_Pedido: o.order_date || "",
          Prazo_Entrega: o.delivery_date || "",
          Itens_Catalogo: prods,
          Observacoes: o.notes || "",
      ...
      // Catalogo
      addSheet("Catalogo", catalog.map((c) => ({
        Nome: c.name || "",
        Categoria: c.category || "",
        Preco: c.price || 0,
        Peso_g: c.weight || 0,
        Estoque: c.in_stock || 0,
        Descricao: c.description || "",
        Imagem_URL: c.image_url || "",
        Criado_em: c.created_at || "",
      })));

      // Insumos
      addSheet("Insumos", supplies.map((i) => ({
        Nome: i.name || "",
        Tipo: (i.kind || "consumivel") === "equipamento" ? "Equipamento" : "Consumivel",
        Categoria: i.category || "",
        Preco_Unitario: i.price || 0,
        Quantidade: i.quantity || 1,
        Total: (i.price || 0) * (i.quantity || 1),
        Arquivado: i.archived ? "Sim" : "Nao",
        Observacoes: i.notes || "",
        Criado_em: i.created_at || "",
      })));

      // Servicos
      addSheet("Servicos", services.map((s) => ({
        Nome: s.name || "",
      ...
      XLSX.writeFile(wb, `backup-layerlab-${aaaa}-${mm}-${dd}.xlsx`);
    } catch (err) {
      ...
      <div style={s.content}>
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
            <div style={{ flex: 1 }}>
              <p style={{ margin: "0 0 2px", fontWeight: 700, fontSize: 14 }}>💾 Backup dos dados</p>
              <p style={{ margin: 0, fontSize: 12, color: C.textSoft }}>
                Baixa um Excel com clientes, pedidos, catalogo, insumos e servicos. Salve no Google Drive.
              </p>
            </div>
          </div>
          <button
            style={{ ...s.btn, width: "100%", justifyContent: "center", marginTop: 10, background: "#3D5AFE" }}
            disabled={exporting}
            onClick={exportBackup}
          >
            {exporting ? "Gerando..." : "⬇️ Exportar Backup (Excel)"}
          </button>
        </div>
  ...
  function renderPage() {
    switch (page) {
      ...
      case "services":
        return <ServicesPage />;
      case "finance":
        return <FinancePage orders={orders} supplies={supplies} clients={clients} catalog={catalog} services={services} />;
      default:
        return null;
    }
  }
