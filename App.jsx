...
function CalcPage({ services = [], reloadServices, orders = [], reloadOrders, setPage }) {
  ...
  async function linkToOrder(orderId) {
    ...
    try {
      ...
      await db.update("orders", orderId, { final_price: result.price, profit: pieceProfit });
...
function OrdersPage({ clients, catalog, services: svcList, reloadOrders }) {
  ...
  const load = useCallback(async () => {
    ...
    setOrders(data);
    setLoading(false);
    if (reloadOrders) reloadOrders(); // atualiza dashboard/financeiro
  }, [reloadOrders]);
...
function SuppliesPage({ reloadSupplies }) {
  ...
  const load = useCallback(async () => {
    ...
    setItems(data);
    if (reloadSupplies) reloadSupplies(); // atualiza dashboard/financeiro
  }, [reloadSupplies]);
