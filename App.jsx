...
const NAV_ITEMS = [
  ...
  { id: "calc", icon: "calc", label: "Precos" },
...
function HomePage({ setPage, orders, supplies }) {
  ...
  const tiles = [
    { id: "calc", icon: "calc", label: "Precos", tint: "#9B6DC5" },
...
function CalcPage({ services = [], reloadServices, orders = [], reloadOrders, setPage }) {
  ...
  async function linkToOrder(orderId) {
    ...
    setLinking(true);
    try {
      await db.update("orders", orderId, { final_price: result.price });
              ...
              <>
                {ordersOpen.length === 0 ? (
                  <p style={{ fontSize: 12, color: C.textSoft, margin: 0 }}>
                    Nenhum pedido em aberto. Crie um pedido primeiro.
                  </p>
                ) : (
                  <select
                    style={s.input}
                  ...
                  >
                    <option value="">{linking ? "Vinculando..." : "Escolha um pedido..."}</option>
                    {ordersOpen.map((o) => (
...
function OrderEdit({ order, clients, catalog, services, onSave, onBack }) {
  ...
  function handleWhatsApp() {
    ...
    const finalValue =
      form.final_price != null && form.final_price !== ""
        ? parseFloat(form.final_price)
        : grandTotal;
    const lines = [
      ...
      <div style={{ ...s.content, paddingTop: 12 }}>
        ...
        {tab === "info" && (
          <div>
            ...
            >
              <MoneyField
                label="Valor Final (pos-producao)"
                value={form.final_price === "" ? 0 : form.final_price}
                onChange={(v) => setForm((f) => ({ ...f, final_price: v }))}
                placeholder="0,00"
              />
              <p style={{ margin: "2px 2px 10px", fontSize: 11, color: C.textSoft }}>
                Este e o valor que vai no WhatsApp. Se deixar zerado, usa a soma dos itens ({fmtBRL(grandTotal)}).
              </p>
              <button
              ...
              >
                💬 Avisar valor e prazo ao cliente
              </button>
            </div>
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8, alignItems: "center" }}>
          ...
          <span style={{ fontWeight: 700, color: "#69F0AE" }}>
            {fmtBRL(form.final_price !== "" && form.final_price != null ? form.final_price : grandTotal)}
          </span>
        </div>
        {savedOk && (
    ...
    <div style={s.page}>
      ...
      <div style={s.content}>
            ...
            {items.map((item) => (
              <div key={item.id} style={{ ...s.card, padding: 0, overflow: "hidden" }}>
                <div style={{ cursor: "pointer" }} onClick={() => setPreview(item)}>
                  {item.image_url ? (
                    <img src={item.image_url} alt={item.name} style={{ width: "100%", height: 120, objectFit: "cover", display: "block" }} onError={(e) => { e.target.style.display = "none"; }} />
                  ) : (
                    <div style={{ height: 120, background: C.dark, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 30, color: C.textSoft }}>📦</div>
                  )}
                </div>
                <div style={{ padding: 10 }}>
                  <div style={{ cursor: "pointer" }} onClick={() => setPreview(item)}>
            ...
            {items.map((item) => (
              ...
              >
                {item.image_url ? (
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", fontSize: 30, color: C.textSoft }}>📦</div>
                )}
        ...
        {!showNew && viewMode === "list" && (
          <div>
            {items.map((item) => (
              <div key={item.id} style={{ ...s.card, padding: 12, display: "flex", gap: 12, alignItems: "center" }}>
                {item.image_url ? (
                  <img src={item.image_url} alt={item.name} style={{ width: 56, height: 56, objectFit: "cover", borderRadius: 10, flexShrink: 0 }} />
                ) : (
                  <div style={{ width: 56, height: 56, borderRadius: 10, background: C.dark, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, flexShrink: 0 }}>📦</div>
                )}
                <div style={{ flex: 1, minWidth: 0, cursor: "pointer" }} onClick={() => setPreview(item)}>
                  <p style={{ margin: "0 0 1px", fontWeight: 700, fontSize: 15 }}>{item.name}</p>
          ...
          >
            {preview.image_url ? (
              <img
                ...
                alt={preview.name}
                style={{ width: "100%", maxWidth: 420, borderRadius: 16, marginBottom: 18 }}
              />
            ) : (
              ...
              <div style={{ display: "flex", gap: 16, marginBottom: 14, flexWrap: "wrap" }}>
                ...
                {preview.in_stock > 0 && (
                  <div>
                    ...
                    <p style={{ margin: 0, fontSize: 22, fontWeight: 700, fontFamily: DISPLAY }}>{preview.in_stock}</p>
                  </div>
                )}
              </div>
    ...
    <div style={s.page}>
      ...
      <div style={s.content}>
        ...
        <div style={{ ...s.card, background: "rgba(198,40,40,0.12)", border: "1px solid #C6282855", marginBottom: 12 }}>
          <p style={{ margin: 0, fontSize: 12, color: C.textSoft }}>Total Investido (tudo)</p>
          <p style={{ margin: "4px 0 0", fontSize: 26, fontWeight: 700, fontFamily: DISPLAY, letterSpacing: "-0.5px", color: "#FF8A80" }}>{fmtBRL(totalGeral)}</p>
          <div style={{ display: "flex", gap: 16, marginTop: 8 }}>
            <span style={{ fontSize: 12, color: C.textSoft }}>Equipamentos: <b style={{ color: C.text }}>{fmtBRL(totalEquip)}</b></span>
            <span style={{ fontSize: 12, color: C.textSoft }}>Consumiveis: <b style={{ color: C.text }}>{fmtBRL(totalConsum)}</b></span>
          </div>
        </div>
...
export default function App() {
  ...
  function renderPage() {
    switch (page) {
      ...
      case "calc":
        return <CalcPage services={services} reloadServices={reloadServices} orders={orders} reloadOrders={reloadOrders} setPage={setPage} />;
      case "orders":
