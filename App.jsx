...
// SEÇÃO: PEDIDOS
// ════════════════════════════════════════════════════════
const Orders = ({ orders, addOrder, updateOrder, deleteOrder, updateOrderStatus, clients, addClient, catalog }) => {
  const [showModal, setShowModal] = useState(false);
  const [showInvoice, setShowInvoice] = useState(null); // pedido selecionado p/ faturar
  const [showNewClient, setShowNewClient] = useState(false);
  const [editing, setEditing] = useState(null);
  const [saving, setSaving] = useState(false);
  const statuses = ["Pedido Recebido", "Em Produção", "Acabamento", "Finalizado", "Entregue"];
  const today = new Date().toISOString().split("T")[0];
  const emptyForm = { clientName: "", description: "", orderDate: today, deliveryDate: "", status: "Pedido Recebido", notes: "", finalPrice: "0", paymentMethod: "PIX", paid: false };
  ...
  const [invoicePayment, setInvoicePayment] = useState("PIX");
  const [invoiceDiscount, setInvoiceDiscount] = useState("0");

  const openNew = () => { setForm(emptyForm); setEditing(null); setShowModal(true); setShowNewClient(false); };
  const openEdit = (o) => {
    setForm({ clientName: o.clientName||"", description: o.description||"", orderDate: o.orderDate||today, deliveryDate: o.deliveryDate||"", status: o.status||"Pedido Recebido", notes: o.notes||"", finalPrice: o.finalPrice||"0", paymentMethod: o.paymentMethod||"PIX", paid: o.paid||false });
    setEditing(o.id); setShowModal(true); setShowNewClient(false);
  };
  ...
  const saveNewClient = async () => {
    ...
    setForm(f => ({ ...f, clientName: newClientForm.name }));
  ...
  const openInvoice = (o) => {
    setShowInvoice(o);
    setInvoiceItems(o.invoiceItems || []);
    setInvoicePayment(o.paymentMethod || "PIX");
    setInvoiceDiscount("0");
  };

  const addInvoiceItem = (catalogItem) => {
    const existing = invoiceItems.find(i => i.id === catalogItem.id);
    if (existing) {
      setInvoiceItems(items => items.map(i => i.id === catalogItem.id ? { ...i, qty: i.qty + 1 } : i));
    } else {
      setInvoiceItems(items => [...items, { ...catalogItem, qty: 1 }]);
    }
  };

  const invoiceSubtotal = invoiceItems.reduce((s, i) => s + parseFloat(i.price) * i.qty, 0);
  const invoiceTotal = Math.max(0, invoiceSubtotal - parseFloat(invoiceDiscount || 0));

  const saveInvoice = async () => {
    if (!showInvoice) return;
    setSaving(true);
    await updateOrder(showInvoice.id, {
      ...showInvoice,
      finalPrice: invoiceTotal,
  ...
  const sendWhatsApp = (order, client) => {
    const msgs = {
      "Pedido Recebido": `Olá ${order.clientName}! 😊 Recebemos seu pedido na LayerLab! Em breve começamos a produção. Qualquer dúvida é só chamar! ✨`,
      "Em Produção": `Olá ${order.clientName}! Seu pedido já está em produção! 🖨️ Assim que terminar avisamos. #LayerLab`,
      "Acabamento": `Olá ${order.clientName}! Seu pedido está na fase de acabamento! 🎨 Logo fica pronto!`,
      "Finalizado": `Olá ${order.clientName}! 🎉 Seu pedido está finalizado e pronto pra entrega! Vamos combinar os detalhes? 📦`,
    };
    const msg = msgs[order.status] || `Olá ${order.clientName}! Atualização: *${order.status}*. Dúvidas? É só chamar!`;
  ...
  return (
    <div>
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {[...filtered].reverse().map(o => {
            ...
            return (
              <div key={o.id} style={{ background: C.white, borderRadius: 14, padding: "16px 18px", boxShadow: "0 1px 8px rgba(123,79,166,0.08)" }}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
                  <select value={o.status} onChange={e => {
                    if (e.target.value === "Entregue" && !o.paid) {
                      alert("⚠️ Pagamento em aberto! Confirme o recebimento antes de marcar como entregue.");
                      return;
                    }
                  ...
                  <button onClick={() => updateOrderStatus(o.id + "_paid", !o.paid, o)} style={{
                    padding: "6px 14px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer",
                    border: `2px solid ${o.paid ? "#2E7D32" : "#E0A800"}`,
                    background: o.paid ? "#D4EDDA" : "#FFF3CD",
                    color: o.paid ? "#2E7D32" : "#B07800"
                  }}>
                    {o.paid ? "✅ Pago" : "💰 Em aberto"}
                  </button>
                  <button onClick={() => openInvoice(o)} style={{
                    padding: "6px 14px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer",
                    border: `2px solid ${C.primary}`, background: C.lavender, color: C.primary, display: "flex", alignItems: "center", gap: 4
                  }}>
                    🧾 Faturar
                  </button>
      ...
      {/* MODAL NOVO/EDITAR PEDIDO — simplificado */}
      {showModal && (
        <Modal title={editing ? "Editar Pedido" : "Novo Pedido"} onClose={() => setShowModal(false)}>
          <ClientSearch clients={clients} value={form.clientName} onChange={v => { setForm(f => ({ ...f, clientName: v })); setShowNewClient(false); }} />

          {/* Botão cadastrar novo cliente */}
          {!showNewClient && form.clientName && !clients.find(c => c.name.toLowerCase() === form.clientName.toLowerCase()) && (
            <button onClick={() => { setNewClientForm({ name: form.clientName, whatsapp: "", email: "" }); setShowNewClient(true); }}
              style={{ marginBottom: 14, padding: "8px 14px", borderRadius: 10, border: `1.5px dashed ${C.primary}`, background: C.lavender, color: C.primary, fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", width: "100%" }}>
              ➕ Cadastrar "{form.clientName}" como novo cliente
            </button>
          )}
          ...
          {showNewClient && (
            <div style={{ background: C.lavender, borderRadius: 12, padding: 14, marginBottom: 14, border: `1.5px solid ${C.lavenderMid}` }}>
              ...
              <Field label="Nome *" value={newClientForm.name} onChange={v => setNewClientForm(f => ({ ...f, name: v }))} placeholder="Nome do cliente" />
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <Btn onClick={saveNewClient} disabled={!newClientForm.name.trim()}><Icon name="check" size={14} color="#fff" /> Salvar Cliente</Btn>
                <Btn variant="secondary" onClick={() => setShowNewClient(false)}>Cancelar</Btn>
              </div>
            </div>
          )}

          <Field label="Descrição do Pedido" value={form.description} onChange={v => setForm(f => ({ ...f, description: v }))} placeholder="Detalhes, referências, cores..." />
          ...
          <Field label="Observações internas" value={form.notes} onChange={v => setForm(f => ({ ...f, notes: v }))} placeholder="Anotações..." />
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 8 }}>
            <Btn variant="secondary" onClick={() => setShowModal(false)}>Cancelar</Btn>
            <Btn onClick={save} disabled={!form.clientName.trim() || saving}>
      ...
      {/* MODAL FATURAMENTO */}
      {showInvoice && (
        <Modal title={`🧾 Faturar — ${showInvoice.clientName}`} onClose={() => setShowInvoice(null)}>
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: C.textMid, marginBottom: 8, textTransform: "uppercase", letterSpacing: 0.5 }}>Adicionar do Catálogo</label>
            <div style={{ display: "flex", flexDirection: "column", gap: 6, maxHeight: 180, overflowY: "auto" }}>
              {catalog.length === 0 ? (
                <p style={{ color: C.textLight, fontSize: 13 }}>Nenhum item no catálogo ainda.</p>
              ) : catalog.map(item => (
                <div key={item.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", borderRadius: 8, background: C.lavender, border: `1px solid ${C.lavenderMid}` }}>
                  <div>
                  ...
                  <button onClick={() => addInvoiceItem(item)} style={{ padding: "4px 12px", borderRadius: 8, border: "none", background: C.primary, color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>+ Add</button>
                </div>
              ))}
            </div>
          </div>
          ...
          {invoiceItems.length > 0 && (
            <div style={{ marginBottom: 14 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: C.textMid, marginBottom: 8, textTransform: "uppercase", letterSpacing: 0.5 }}>Itens do Pedido</label>
              {invoiceItems.map((item, i) => (
                <div key={item.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", marginBottom: 6, borderRadius: 8, background: C.bg, border: `1px solid ${C.lavenderMid}` }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 600, fontSize: 13 }}>{item.name}</div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 4 }}>
                      <button onClick={() => setInvoiceItems(its => its.map((it,j) => j===i ? {...it, qty: Math.max(1, it.qty-1)} : it))} style={{ width: 22, height: 22, borderRadius: 6, border: "none", background: C.lavenderMid, cursor: "pointer", fontWeight: 700 }}>-</button>
                      <span style={{ fontWeight: 700, fontSize: 13 }}>{item.qty}</span>
                      <button onClick={() => setInvoiceItems(its => its.map((it,j) => j===i ? {...it, qty: it.qty+1} : it))} style={{ width: 22, height: 22, borderRadius: 6, border: "none", background: C.lavenderMid, cursor: "pointer", fontWeight: 700 }}>+</button>
                  ...
                  <div style={{ textAlign: "right" }}>
                    ...
                    <button onClick={() => setInvoiceItems(its => its.filter((_,j) => j!==i))} style={{ fontSize: 11, color: C.danger, background: "none", border: "none", cursor: "pointer", padding: 0, marginTop: 2 }}>remover</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <Field label="Desconto (R$)" type="number" value={invoiceDiscount} onChange={setInvoiceDiscount} placeholder="0,00" step="0.01" />
          <Select label="Forma de Pagamento" value={invoicePayment} onChange={setInvoicePayment} options={["PIX", "Cartão de Crédito", "Cartão de Débito", "Dinheiro", "Transferência", "A Combinar"]} />

          {/* TOTAL */}
          <div style={{ background: `linear-gradient(135deg, ${C.primaryDark}, ${C.primary})`, borderRadius: 12, padding: "16px 18px", marginBottom: 16 }}>
            {parseFloat(invoiceDiscount) > 0 && (
              <div style={{ display: "flex", justifyContent: "space-between", color: "rgba(255,255,255,0.7)", fontSize: 13, marginBottom: 6 }}>
                <span>Subtotal</span><span>R$ {invoiceSubtotal.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</span>
              </div>
            )}
            {parseFloat(invoiceDiscount) > 0 && (
              <div style={{ display: "flex", justifyContent: "space-between", color: "#FFD166", fontSize: 13, marginBottom: 6 }}>
                <span>Desconto</span><span>- R$ {parseFloat(invoiceDiscount).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</span>
              </div>
            )}
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ color: "#fff", fontWeight: 700, fontSize: 15 }}>💰 Total</span>
              <span style={{ color: "#fff", fontWeight: 900, fontSize: 22 }}>R$ {invoiceTotal.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</span>
            </div>
            <div style={{ color: "rgba(255,255,255,0.6)", fontSize: 12, marginTop: 4 }}>💳 {invoicePayment}</div>
          </div>

          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
            <Btn variant="secondary" onClick={() => setShowInvoice(null)}>Cancelar</Btn>
            <Btn onClick={saveInvoice} disabled={invoiceItems.length === 0 || saving}>
              <Icon name="check" size={15} color="#fff" /> {saving ? "Salvando..." : "Confirmar Faturamento"}
            </Btn>
          </div>
        </Modal>
      )}
  ...
  const openEdit = (o) => { setForm({ clientName: o.clientName||"", productName: o.productName||"", description: o.description||"", orderDate: o.orderDate||today, deliveryDate: o.deliveryDate||"", paymentMethod: o.paymentMethod||"PIX", finalPrice: o.finalPrice||"", status: o.status||"Pedido Recebido", paid: o.paid||false, notes: o.notes||"" }); setEditing(o.id); setShowModal(true); };
  const save = async () => {
  ...
  const filtered = orders.filter(o => filterStatus === "Todos" || o.status === filterStatus || (filterStatus === "Finalizado" && o.status === "Pagamento Recebido"));
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
        {["Todos", "Pedido Recebido", "Em Produção", "Acabamento", "Finalizado", "Entregue"].map(s => (
          <button key={s} onClick={() => setFilterStatus(s)} style={{
            padding: "6px 14px", borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: "pointer",
            background: filterStatus === s ? C.primary : C.lavender,
      ...
      {showModal && (
        <Modal title={editing ? "Editar Pedido" : "Novo Pedido"} onClose={() => setShowModal(false)}>
          ...
          <ClientSearch clients={clients} value={form.clientName} onChange={v => setForm(f => ({ ...f, clientName: v }))} />
          ...
          <Select label="Status" value={form.status} onChange={v => setForm(f => ({ ...f, status: v }))} options={["Pedido Recebido", "Em Produção", "Acabamento", "Finalizado", "Entregue"]} />
          <div onClick={() => setForm(f => ({ ...f, paid: !f.paid }))} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14, cursor: "pointer", padding: "10px 14px", borderRadius: 10, background: form.paid ? "#D4EDDA" : "#FFF9E6", border: `1.5px solid ${form.paid ? "#C8E6C9" : "#FFE082"}` }}>
            <div style={{ width: 22, height: 22, borderRadius: 6, background: form.paid ? "#2E7D32" : "transparent", border: `2px solid ${form.paid ? "#2E7D32" : "#B07800"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              {form.paid && <span style={{ color: "#fff", fontSize: 13 }}>✓</span>}
            </div>
            <div>
              <div style={{ fontWeight: 700, fontSize: 14, color: form.paid ? "#2E7D32" : "#B07800" }}>{form.paid ? "💰 Pagamento recebido" : "⏳ Aguardando pagamento"}</div>
...
export default function App() {
  ...
  const toDb = (o) => ({ client_name: o.clientName, product_name: o.productName||"", description: o.description, order_date: o.orderDate, delivery_date: o.deliveryDate, payment_method: o.paymentMethod||"PIX", final_price: parseFloat(o.finalPrice) || 0, status: o.status, paid: o.paid||false, notes: o.notes, invoice_items: o.invoiceItems||null });
  const fromDb = (o) => ({ ...o, clientName: o.client_name, productName: o.product_name, orderDate: o.order_date, deliveryDate: o.delivery_date, paymentMethod: o.payment_method, finalPrice: o.final_price, paid: o.paid||false });
  ...
  const renderPage = () => {
    switch (page) {
      ...
      case "orders":     return <Orders orders={orders} addOrder={addOrder} updateOrder={updateOrder} deleteOrder={deleteOrder} updateOrderStatus={updateOrderStatus} clients={clients} addClient={addClient} catalog={catalog} />;
