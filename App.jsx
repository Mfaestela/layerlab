...
const statusConfig = {
  ...
  "Em Produo":     { color: "#F0A500", bg: "#FFF3CD", emoji: "" },
...
const Dashboard = ({ orders, clients, catalog, supplies, setPage }) => {
  ...
  const emProducao = orders.filter(o => o.status === "Em Produo").length;
  ...
  const navButtons = [
    ...
    { id: "catalog",    label: "Catlogo",    icon: "catalog" },
    ...
    <div style={{ paddingBottom: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginBottom: 24 }}>
        {[
          ...
          { label: "Produo",   value: emProducao, color: "#FFD166", dot: "#E0A800" },
...
const Calculator = () => {
  ...
  return (
    <div>
      <h2 style={{ color: C.primaryDark, margin: "0 0 4px" }}>Calculadora de Precificao</h2>
      <p style={{ color: C.textMid, marginBottom: 24 }}>Calcule o preo ideal para cada pea.</p>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <div style={{ background: C.white, borderRadius: 14, padding: 22, boxShadow: "0 2px 12px rgba(123,79,166,0.08)" }}>
          ...
          <Field label="Preo do Filamento (R$/kg)" type="number" value={filamentPrice} onChange={v => { setFilamentPrice(v); setCalculated(false); }} placeholder="90" />
          <Field label="Gramas Usadas (g)" type="number" value={gramsUsed} onChange={v => { setGramsUsed(v); setCalculated(false); }} placeholder="50" />
          <h4 style={{ color: C.primaryDark, margin: "16px 0 12px", fontSize: 13, textTransform: "uppercase", letterSpacing: 1 }}> Energia & Mquina</h4>
          <Field label="Horas de Impresso (h)" type="number" value={printHours} onChange={v => { setPrintHours(v); setCalculated(false); }} placeholder="3" step="0.5" />
          ...
          <Field label="Desgaste Mq. por Hora (R$/h)" type="number" value={machinePerHour} onChange={v => { setMachinePerHour(v); setCalculated(false); }} placeholder="1.50" step="0.01" />
        </div>
        <div style={{ background: C.white, borderRadius: 14, padding: 22, boxShadow: "0 2px 12px rgba(123,79,166,0.08)" }}>
          <h4 style={{ color: C.primaryDark, margin: "0 0 16px", fontSize: 13, textTransform: "uppercase", letterSpacing: 1 }}> Mo de Obra</h4>
      ...
      <button onClick={calculate} style={{
        ...
        {calculated ? " Calculado!" : " Calcular Preo"}
      </button>
      ...
      {result && (
        <div style={{ background: `linear-gradient(135deg, ${C.primaryDark}, ${C.primary})`, borderRadius: 14, padding: 24, marginTop: 12, color: "#fff" }}>
          ...
          {row(" Mo de obra", result.laborCost)}
          ...
          <div style={{
            ...
            <span style={{ fontSize: 14, fontWeight: 600 }}> Preo Sugerido</span>
    ...
    <div>
      ...
      {showModal && (
        <Modal title={editing ? "Editar Cliente" : "Novo Cliente"} onClose={() => setShowModal(false)}>
          ...
          <Field label="WhatsApp (s nmeros)" value={form.whatsapp} onChange={v => setForm(f => ({ ...f, whatsapp: v }))} placeholder="11999999999" />
          <Field label="E-mail" value={form.email} onChange={v => setForm(f => ({ ...f, email: v }))} placeholder="email@exemplo.com" />
          <Field label="Observaes" value={form.notes} onChange={v => setForm(f => ({ ...f, notes: v }))} placeholder="Preferncias, histrico..." />
...
const ServicesCatalog = ({ services, addService, deleteService }) => {
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          <h2 style={{ color: C.primaryDark, margin: 0 }}>Servios</h2>
          <p style={{ color: C.textMid, margin: "4px 0 0" }}>{services.length} servio{services.length !== 1 ? "s" : ""}</p>
        </div>
        <Btn onClick={() => setShowModal(true)}><Icon name="plus" size={16} color="#fff" /> Novo Servio</Btn>
      </div>

      {services.length === 0 ? (
        <div style={{ textAlign: "center", padding: "40px 0", color: C.textLight }}>
          <div style={{ fontSize: 40, marginBottom: 8 }}></div>
          <p>Nenhum servio ainda. Adicione lixamento, pintura, montagem...</p>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {services.map(s => (
            <div key={s.id} style={{ background: C.white, borderRadius: 12, padding: "14px 18px", display: "flex", justifyContent: "space-between", alignItems: "center", boxShadow: "0 1px 6px rgba(123,79,166,0.07)" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <button onClick={() => { if(confirm("Remover servio?")) deleteService(s.id); }} style={{ padding: "5px 10px", borderRadius: 8, background: C.danger, color: "#fff", border: "none", cursor: "pointer", fontSize: 13, fontWeight: 700 }}></button>
              </div>
            </div>
          ))}
        </div>
      ...
      {showModal && (
        <Modal title="Novo Servio" onClose={() => setShowModal(false)}>
          <Field label="Nome do Servio *" value={form.name} onChange={v => setForm(f => ({ ...f, name: v }))} placeholder="Ex: Lixamento, Pintura, Montagem..." />
          <Field label="Descrio" value={form.description} onChange={v => setForm(f => ({ ...f, description: v }))} placeholder="Detalhes do servio..." />
...
const Orders = ({ orders, addOrder, updateOrder, deleteOrder, updateOrderStatus, clients, addClient, catalog, services }) => {
  ...
  const statuses = ["Pedido Recebido", "Em Produo", "Acabamento", "Finalizado", "Entregue"];
  ...
  const saveNew = async () => {
    ...
    // Abre edio do pedido recm criado
    setTimeout(() => {
  ...
  const sendWhatsApp = (order, client) => {
    const msgs = {
      "Pedido Recebido": `Ol ${order.clientName}!  Recebemos seu pedido na LayerLab! Em breve comeamos a produo. `,
      "Em Produo": `Ol ${order.clientName}! Seu pedido est em produo! `,
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", minHeight: "calc(100vh - 140px)" }}>
        ...
        <div style={{ display: "flex", gap: 4, marginBottom: 20, borderBottom: "2px solid rgba(201,168,255,0.15)" }}>
          {[
            { id: "info", label: " Informaes" },
            { id: "products", label: " Produtos" },
            { id: "services", label: " Servios" },
          ].map(tab => (
        ...
        <div style={{ flex: 1, marginBottom: 80 }}>
          {activeTab === "info" && (
            <div>
              ...
              <Field label="Descrio" value={editingOrder.description||""} onChange={v => setEditingOrder(o => ({ ...o, description: v }))} placeholder="Detalhes, referncias, cores..." />
              ...
              <Select label="Forma de Pagamento" value={editingOrder.paymentMethod||"PIX"} onChange={v => setEditingOrder(o => ({ ...o, paymentMethod: v }))} options={["PIX", "Carto de Crdito", "Carto de Dbito", "Dinheiro", "Transferncia", "A Combinar"]} />
              ...
              <Field label="Observaes" value={editingOrder.notes||""} onChange={v => setEditingOrder(o => ({ ...o, notes: v }))} placeholder="Anotaes internas..." />
            </div>
          )}
        ...
        <div style={{ position: "fixed", bottom: 60, left: 0, right: 0, zIndex: 100, display: "flex", justifyContent: "center" }}>
          <div style={{ display: "flex", gap: 10, padding: "12px 20px", background: "rgba(20,10,48,0.95)", backdropFilter: "blur(16px)", borderTop: "1px solid rgba(201,168,255,0.15)", borderRadius: "16px 16px 0 0", width: "100%", maxWidth: 600, boxSizing: "border-box" }}>
            ...
            const msg = `Ol ${editingOrder.clientName}! \n\nSegue o resumo do seu pedido na *LayerLab* \n\n`
              + ` *Pedido:* ${editingOrder.description || "Impresso 3D"}\n`
              ...
              + `\nQualquer dvida  s chamar! \n_LayerLab  Gesto de Impresso 3D_`;
            const phone = client?.whatsapp?.replace(/\D/g,"") || "";
    ...
    <div>
      ...
      {showNewModal && (
        <Modal title="Novo Pedido" onClose={() => setShowNewModal(false)}>
          ...
          <Field label="Descrio" value={newForm.description} onChange={v => setNewForm(f => ({ ...f, description: v }))} placeholder="Detalhes do pedido..." />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          ...
          <Field label="Observaes" value={newForm.notes} onChange={v => setNewForm(f => ({ ...f, notes: v }))} placeholder="Anotaes..." />
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 8 }}>
...
const Catalog = ({ catalog, addCatalog, updateCatalog, deleteCatalog }) => {
  ...
  const [editing, setEditing] = useState(null);
  const [saving, setSaving] = useState(false);
  const emptyForm = { name: "", description: "", price: "", category: "Decorao", inStock: "1", imageUrl: "" };
  const [form, setForm] = useState(emptyForm);
  const [shareText, setShareText] = useState("");

  const openNew = () => { setForm(emptyForm); setEditing(null); setShowModal(true); };
  const openEdit = (p) => { setForm({ name: p.name||"", description: p.description||"", price: p.price||"", category: p.category||"Decorao", inStock: p.inStock||"1", imageUrl: p.imageUrl||"" }); setEditing(p.id); setShowModal(true); };
  const save = async () => {
    if (!form.name.trim()) return;
    setSaving(true);
  ...
  const generatePortfolio = () => {
    const text = catalog.map(p => ` *${p.name}*\n${p.description}\nPreo: R$ ${parseFloat(p.price).toFixed(2).replace(".", ",")}\n`).join("\n");
    const msg = ` *LayerLab*  Portflio de Produtos\n\n${text}\nPedidos e informaes pelo WhatsApp! `;
    setShareText(msg);
  };
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          <h2 style={{ color: C.primaryDark, margin: 0 }}>Catlogo & Portflio</h2>
          <p style={{ color: C.textMid, margin: "4px 0 0" }}>{catalog.length} produto{catalog.length !== 1 ? "s" : ""}</p>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          {catalog.length > 0 && <Btn variant="secondary" onClick={generatePortfolio}> Gerar Portflio</Btn>}
      ...
      {shareText && (
        <div style={{ background: C.lavender, borderRadius: 12, padding: 16, marginBottom: 16 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: C.primaryDark, marginBottom: 8 }}> Portflio gerado  copie e envie pelo WhatsApp:</div>
      ...
      {catalog.length === 0 ? (
        <div style={{ textAlign: "center", padding: "40px 0", color: C.textLight }}>
          <div style={{ fontSize: 40, marginBottom: 8 }}></div>
          <p>Nenhum produto ainda. Adicione peas prontas pro catlogo!</p>
        </div>
      ) : (
      ...
      {showModal && (
        <Modal title={editing ? "Editar Produto" : "Novo Produto"} onClose={() => setShowModal(false)}>
          <Field label="Nome do Produto *" value={form.name} onChange={v => setForm(f => ({ ...f, name: v }))} placeholder="Ex: Miniatura de drago" />
          <Field label="Descrio" value={form.description} onChange={v => setForm(f => ({ ...f, description: v }))} placeholder="Material, tamanho, cores disponveis..." />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <MoneyField label="Preo (R$)" value={form.price} onChange={v => setForm(f => ({ ...f, price: v }))} />
            <Field label="Estoque" type="number" value={form.inStock} onChange={v => setForm(f => ({ ...f, inStock: v }))} min="0" />
          </div>
          <Select label="Categoria" value={form.category} onChange={v => setForm(f => ({ ...f, category: v }))} options={["Decorao", "Miniatura", "Funcional", "Personalizado", "Jogo & RPG", "Infantil", "Outro"]} />
...
const LoginScreen = ({ onLogin }) => {
  ...
  const tryLogin = async () => {
    ...
    try {
      ...
      setError("Erro de conexo. Tente novamente.");
    }
    setLoading(false);
  };
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
        <div>
          ...
          <p style={{ color: C.textMid, margin: "4px 0 0", fontSize: 13 }}>Tudo que voc j gastou no negcio</p>
        </div>
      ...
      {showModal && (
        <Modal title="Novo Insumo" onClose={() => setShowModal(false)}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <MoneyField label="Preo Unitrio (R$) *" value={form.price} onChange={v => setForm(f => ({ ...f, price: v }))} />
            <Field label="Quantidade" type="number" value={form.quantity} onChange={v => setForm(f => ({ ...f, quantity: v }))} placeholder="1" min="1" />
          </div>
          <Field label="Observaes" value={form.notes} onChange={v => setForm(f => ({ ...f, notes: v }))} placeholder="Ex: Comprado na Amazon" />
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 8 }}>
            <Btn variant="secondary" onClick={() => setShowModal(false)}>Cancelar</Btn>
            <Btn onClick={save} disabled={!form.name.trim() || !form.price || saving}>
              <Icon name="check" size={15} color="#fff" /> {saving ? "Salvando..." : "Salvar"}
...
<head>
<meta charset="UTF-8"/>
<title>LayerLab  Relatrio Financeiro</title>
<style>
...
<body>
  <div class="header">
    ...
    <div class="date">Relatrio Financeiro  ${dateStr}</div>
  </div>

  <div class="situation">
    <div>
      <h2>${profit ? " No Lucro!" : " Prejuzo"}</h2>
      <div class="amount">${realProfit < 0 ? "-" : ""}${fmt(realProfit)}</div>
      <div style="font-size:12px;color:#555;margin-top:4px">${realProfit < 0 ? `Faltam ${fmt(Math.abs(realProfit))} pra empatar` : "Lucro real aps descontar investimentos"}</div>
    </div>
  ...
  <div class="footer">LayerLab  Gesto de Impresso 3D  Relatrio gerado em ${dateStr}</div>
</body>
</html>`;
    ...
    <div style={{ paddingBottom: 20 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          ...
          <p style={{ color: "rgba(201,168,255,0.5)", margin: "4px 0 0", fontSize: 13 }}>Viso completa do negcio</p>
        </div>
      ...
      <div style={{ ...glassCard, background: realProfit >= 0 ? "linear-gradient(135deg,rgba(46,125,50,0.45),rgba(67,160,71,0.3))" : "linear-gradient(135deg,rgba(192,57,43,0.45),rgba(231,76,60,0.3))", border: `1px solid ${realProfit>=0?"rgba(76,175,80,0.35)":"rgba(231,76,60,0.35)"}`, padding: "18px 20px", marginBottom: 14, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.7)", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1 }}>{realProfit >= 0 ? " No Lucro!" : " Prejuzo"}</div>
          <div style={{ fontSize: 28, fontWeight: 900, color: "#fff", marginTop: 4 }}>{realProfit < 0 ? "-" : ""}{fmt(realProfit)}</div>
          <div style={{ fontSize: 11, color: "rgba(255,255,255,0.6)", marginTop: 2 }}>{realProfit < 0 ? `Faltam ${fmt(Math.abs(realProfit))} pra empatar` : "Lucro real aps investimentos"}</div>
        </div>
...
export default function App() {
  ...
  const nav = [
    ...
    { id: "supplies",  label: "Insumos",     icon: "box" },
    { id: "financial", label: "Financeiro",  icon: "chart" },
    { id: "services",  label: "Servios",    icon: "calc" },
  ];
