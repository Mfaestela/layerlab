  ...
  const CatalogPicker = ({ type }) => (
    <div>
      <div style={{ fontSize: 12, fontWeight: 600, color: C.textMid, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 10 }}>
        {type === "products" ? "Adicionar Produto do Catálogo" : "Adicionar Serviço do Catálogo"}
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 8, maxHeight: 200, overflowY: "auto", marginBottom: 16 }}>
        {catalog.length === 0 ? (
          <p style={{ color: C.textLight, fontSize: 13 }}>Nenhum item no catálogo.</p>
        ) : catalog.map(item => (
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", minHeight: "calc(100vh - 140px)" }}>
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 12, color: C.textLight, marginBottom: 2 }}>Pedido</div>
          <h2 style={{ color: C.primaryDark, margin: 0, fontSize: 18 }}>{editingOrder.clientName}</h2>
          <StatusBadge status={editingOrder.status} />
        </div>

        {/* Abas */}
        <div style={{ display: "flex", gap: 4, marginBottom: 20, borderBottom: `2px solid ${C.lavenderMid}` }}>
          {[
        ...
        <div style={{ position: "fixed", bottom: 60, left: 0, right: 0, background: C.white, borderTop: `1.5px solid ${C.lavenderMid}`, padding: "12px 20px", display: "flex", gap: 10, justifyContent: "center", zIndex: 100 }}>
          <button onClick={() => {
          ...
          }} style={{ flex: 1, padding: "12px", borderRadius: 12, border: `2px solid ${C.primary}`, background: C.lavender, color: C.primary, fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
            🧾 Faturar
          </button>
          <button onClick={async () => { await saveEdit(); }} disabled={saving} style={{ flex: 1, padding: "12px", borderRadius: 12, border: "none", background: `linear-gradient(135deg, ${C.primaryDark}, ${C.primary})`, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
            {saving ? "Salvando..." : "💾 Salvar"}
          </button>
          <button onClick={() => setView("list")} style={{ flex: 1, padding: "12px", borderRadius: 12, border: `1.5px solid ${C.lavenderMid}`, background: C.white, color: C.textMid, fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
            ✕ Sair
          </button>
        </div>
      </div>
              ...
              <div key={o.id} onClick={() => openEdit(o)} style={{ background: C.white, borderRadius: 14, padding: "16px 18px", boxShadow: "0 1px 8px rgba(123,79,166,0.08)", cursor: "pointer" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, alignItems: "flex-end" }}>
                    ...
                    <button onClick={e => { e.stopPropagation(); del(o.id); }} style={{ padding: "5px 10px", borderRadius: 8, background: "#FFF0F0", color: C.danger, border: `1px solid #FFCDD2`, cursor: "pointer", fontSize: 12 }}>
                      🗑
                    </button>
                  </div>
                </div>
